...
from diffusers import CogVideoXControlNetPipeline, CogVideoXControlNetModel
...
def main():
    ...
    controlnet = CogVideoXControlNetModel.from_transformer(transformer).to(device)
    ...
    for idx, batch in progress_wrap(enumerate(iterable = iter(dataloader), start=0), total=num_steps_per_epoch):
        ...
        lq = F.pad(lq.squeeze(0), (0, pad_w, 0, pad_h), mode='reflect').unsqueeze(0)
        hq = F.pad(hq.squeeze(0), (0, pad_w, 0, pad_h), mode='reflect').unsqueeze(0)

        H_padded, W_padded = lq.shape[-2:]

        # lq_patches = F.unfold(lq.view(B*T, C, H_padded, W_padded), kernel_size=patch_size, stride=stride)
        # hq_patches = F.unfold(hq.view(B*T, C, H_padded, W_padded), kernel_size=patch_size, stride=stride)
        # N_patches = lq_patches.shape[-1]ß
        

        # lq_patches = lq_patches.view(B, T, C, patch_size, patch_size, N_patches).permute(0, 2, 5, 1, 3, 4)
        # hq_patches = hq_patches.view(B, T, C, patch_size, patch_size, N_patches).permute(0, 2, 5, 1, 3, 4) #[B, C, N_patches, T, patch_size, patch_size]
        # 分块操作
        lq_patches = F.unfold(lq.permute(0,2,1,3,4).view(B*T, C, H_padded, W_padded), kernel_size=patch_size, stride=stride)
        N_patches = lq_patches.shape[-1]

        # 恢复维度到 [B, T, C, patch_size, patch_size, N_patches]，保证时间维度在第三个维度
        lq_patches = lq_patches.view(B, T, C, patch_size, patch_size, N_patches).permute(0, 1, 2, 5, 3, 4) #[B, T, C, N_patches, patch_size, patch_size]
        ...
        for i in range(lq_patches.shape[3]):
            ...
            samples = scheduler.sample_CogVideoX_ControlNet(
                model,
                text_encoder=None,
        ...
        print(f'processed_patches.shape = {processed_patches.shape}')
        T = processed_patches.shape[1]
        processed_patches = processed_patches.view(B * T, C * patch_size * patch_size, N_patches)   

        reconstructed = F.fold(processed_patches, output_size=(H_padded, W_padded), kernel_size=patch_size, stride=stride)
        reconstructed = reconstructed.view(B, T, C, H_padded, W_padded)
        ...
        if is_main_process():
            ...
            save_path = save_video(reconstructed[0], save_path)
