...
def generate_video(
    ...
    old_weights = pipe.transformer.patch_embed.proj.weight.clone().detach()
    old_bias = pipe.transformer.patch_embed.proj.bias.clone().detach()
    new_weights = torch.zeros_like(old_weights)
    modified_weights = torch.cat([old_weights, new_weights], dim=1)

    pipe.transformer.patch_embed.proj = nn.Conv2d(32, 1920, kernel_size=(2, 2), stride=2, bias=True)
    pipe.transformer.patch_embed.proj.weight.data = modified_weights
    pipe.transformer.patch_embed.proj.bias.data = old_bias
