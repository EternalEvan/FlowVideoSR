...
# from diffusers.models import CogVideoXControlNetModel
...
def main():
    ...
    # == build CogVideoXControlNet model ==
    pipe = CogVideoXPipeline.from_pretrained(cfg.model_path, torch_dtype=dtype).to(device)
    text_encoder = pipe.text_encoder
    requires_grad(text_encoder, False)
    vae = pipe.vae
    ...
    controlnet = CogVideoXControlNetModel.from_transformer(transformer)
    controlnet.train()

    model = TwoStreamControlNet(base_model=transformer, control_model=controlnet)
        ...
        with tqdm(
            ...
            for step, batch in pbar:
                ...
                with timers["diffusion"] as loss_t:
                    ...
                    loss_dict = scheduler.training_losses_CogVideoX_ControlNet(model,x_start=lq, model_kwargs=model_args, mask=None)
