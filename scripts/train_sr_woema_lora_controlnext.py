...
def main():
    ...
    controlnet = CogVideoXControlNextModel.from_transformer(transformer)
    controlnet.train()
    ...
    # 获取需要优化的参数
    # # 获取需要优化的参数及其名字
    # params = filter(lambda p: p[1].requires_grad, 
    #                 chain(transformer.named_parameters(), controlnet.named_parameters()))

    # # 打印参数名字
    # for idx, (name, _) in enumerate(params):
    #     print(f"Parameter {idx}: {name}")
