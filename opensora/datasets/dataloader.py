...
from .datasets import BatchFeatureDataset, VariableVideoTextDataset, VideoTextDataset,VideoRecurrentTestDataset
from .datasets_v1 import VideoRecurrentTestDataset_v1
from .datasets_v2 import VideoRecurrentTestDataset_v2
from .datasets_codeformer import VideoRecurrentCodeFormerDataset,CombinedVideoDataset
from .datasets_codeformer_video import VideoRecurrentCodeFormerVideoDataset
from .sampler import BatchDistributedSampler, StatefulDistributedSampler, VariableVideoBatchSampler,VideoRecurrentTestSampler
...
def prepare_dataloader(
    ...
    **kwargs,
    ...
    elif isinstance(dataset, VideoRecurrentTestDataset):
        sampler = VideoRecurrentTestSampler(dataset, num_replicas=process_group.size(),
            rank=process_group.rank(), shuffle=shuffle,)
        return (
            DataLoader(
                dataset,
                batch_size=batch_size,
    ...
    elif isinstance(dataset, VideoRecurrentTestDataset_v1):
        sampler = VideoRecurrentTestSampler(dataset, num_replicas=process_group.size(),
            rank=process_group.rank(), shuffle=shuffle,)
        return (
            DataLoader(
    ...
    elif isinstance(dataset, VideoRecurrentTestDataset_v2):
        sampler = VideoRecurrentTestSampler(dataset, num_replicas=process_group.size(),
            rank=process_group.rank(), shuffle=shuffle,)
        return (
            DataLoader(
    ...
    elif isinstance(dataset, VideoRecurrentCodeFormerDataset):
        sampler = VideoRecurrentTestSampler(dataset, num_replicas=process_group.size(),
            rank=process_group.rank(), shuffle=shuffle,)
        return (
            DataLoader(
    ...
    elif isinstance(dataset, VideoRecurrentCodeFormerVideoDataset):
        sampler = VideoRecurrentTestSampler(dataset, num_replicas=process_group.size(),
            rank=process_group.rank(), shuffle=shuffle,)
        return (
            DataLoader(
    ...
    elif isinstance(dataset,CombinedVideoDataset):
        sampler = VideoRecurrentTestSampler(dataset, num_replicas=process_group.size(),
            rank=process_group.rank(), shuffle=shuffle,)
        return (
            DataLoader(
