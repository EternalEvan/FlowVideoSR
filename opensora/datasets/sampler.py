...
class StatefulDistributedSampler(DistributedSampler):
    def __init__(
        ...
        self.seed=seed
        self.bucket_ids = self.dataset.data.apply(
            apply,
            axis=1,
    ...
    def __iter__(self) -> Iterator:
        ...
        for idx in indices:
            ...
            cur_micro_batch = f"{idx}-{real_t}-{real_h}-{real_w}" 
...
class VideoRecurrentTestSampler(DistributedSampler):
    ...
    def load_state_dict(self, state_dict: dict):
        self.index = state_dict["start_index"] + 1
