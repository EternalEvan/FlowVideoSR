...
class BlurPool3D(nn.Module):
    ...
    def forward(self, inp):
        if self.filt_size == 1:
            ...
            # print(f'self.pad_sizes = {self.pad_sizes}')
            padded_inp = self.pad(inp)
            # print(f'padded_inp.shape = {padded_inp.shape}')
            # print(f'self.filt = {self.filt}')
            return F.conv3d(padded_inp, self.filt, stride=self.stride, groups=self.channels)
