...
from opensora.models.stdit.stdit3_sr import STDiT3Block
...
class ControlSTDitBlock(Module):
    def __init__(self, base_block: STDiT3Block, block_index: 0) -> None:
        super().__init__()
    ...
    def forward(
        ...
        if self.block_index == 0:
            ...
            c = self.copied_block(x + c, y, t, mask, x_mask, t0, T, S)
            c_skip = self.after_proj(c)
        else:
            # load from previous c and produce the c for skip connection
            c = self.copied_block(c, y, t, mask, x_mask, t0, T, S)
            c_skip = self.after_proj(c)
...
class ControlPixArtHalf(Module):
    ...
    def __init__(self, base_model: STDiT3SR, copy_blocks_num: int = 13, in_channels=4) -> None:
        super().__init__()
        ...
        for i in range(copy_blocks_num):
            self.controlnet.append(ControlSTDitBlockHalf(base_model.blocks[i], i))
    ...
    def forward_c(self, c):
        ...
        return self.base_model.x_embedder(c) + pos_embed if c is not None else c
    ...
    def forward(self, x, timestep, y, mask=None, data_info=None, c=None, **kwargs):
        ...
        # === get x embed ===
        x = self.x_embedder(x) + pos_embed  # (B, S, D), where S = H * W / patch_size ** 2
...
class ControlPixArtMSHalf(ControlPixArtHalf):
    ...
    def forward(self, x, timestep, y, mask=None, data_info=None, c=None, **kwargs):
        ...
        if mask is not None:
            ...
            y_lens = [y.shape[2]] * y.shape[0]
            y = y.squeeze(1).view(1, -1, x.sha00pe[-1])
