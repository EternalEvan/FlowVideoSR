...
class STDiT3(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # embedding
        self.x_embedder = PatchEmbed3D(config.patch_size, config.in_channels, config.hidden_size)#config.in_channels *2
    ...
    def initialize_weights(self):
        ...
        # 初始化第一个 block 的加倍通道为 0
        first_block = self.spatial_blocks[0]
        nn.init.constant_(first_block.attn.proj.weight, 0)
        nn.init.constant_(first_block.cross_attn.proj.weight, 0)
        nn.init.constant_(first_block.mlp.fc2.weight, 0)
...
def STDiT3_XL_2(from_pretrained=None, **kwargs):
    ...
    if force_huggingface or from_pretrained is not None and not os.path.exists(from_pretrained):
        ...
        # new_x_embedder = PatchEmbed3D(in_chans=8, patch_size=(1, 2, 2), embed_dim=1152)
        # # print(model.x_embedder.proj.weight.data.shape)
        # # print(type(model.x_embedder.proj.weight.data))
        # original_weight_data =model.x_embedder.proj.weight.data
        # new_weight_data = original_weight_data
        # conv3d_layer = nn.Conv3d(in_channels=4, out_channels=1152, kernel_size=(1, 2, 2), stride=(1, 2, 2))
        # import torch.nn.init as init
        # init.kaiming_normal_(conv3d_layer.weight, mode='fan_out', nonlinearity='relu')
        # if conv3d_layer.bias is not None:
        #     init.zeros_(conv3d_layer.bias)
        # new_weight = torch.cat((original_weight_data, conv3d_layer.weight.data), dim=1)
        # # print("3333333333333",new_weight.shape)
        # new_x_embedder.proj.weight.data = new_weight
        # model.x_embedder = new_x_embedder
        # print("4444444444444",model.x_embedder.proj.weight.data.shape)
