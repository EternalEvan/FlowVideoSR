...
class ControlSTDiT3SR(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # build controlnet
        self.controlsptial = []
        self.controltemporal = []
        for p in self.parameters():
            p.requires_grad_(False)
        from opensora.models.controlnet import ControlSTDitBlock
        # Copy first copy_blocks_num block
        for i in range(config.depth//2):
            self.controlsptial.append(ControlSTDitBlock(self.spatial_blocks[i], i))
            self.controltemporal.append(ControlSTDitBlock(self.temporal_blocks[i], i))
        self.controlsptial = nn.ModuleList(self.controlsptial)
        self.controltemporal = nn.ModuleList(self.controltemporal)

        for p in self.controlsptial.parameters():
            p.requires_grad_(True)

        for p in self.controltemporal.parameters():
            p.requires_grad_(True)
    ...
    def forward(self, x, timestep, y, c=None, mask=None, x_mask=None, fps=None, height=None, width=None, **kwargs):
        ...
        if height.dim() == 0:
            resolution_sq = (height.item() * width.item()) ** 0.5
        else:
        ...
        if c is not None:
            c = self.x_embedder(c)  # [B, N, C]
            c = rearrange(c, "B (T S) C -> B T S C", T=T, S=S) 
            c = c + pos_emb
            c = rearrange(c, "B T S C -> B (T S) C", T=T, S=S)  # [B, N, C]
        ...
        x = rearrange(x, "B T S C -> B (T S) C", T=T, S=S)  # [B, N, C]

        # === blocks ===
        if c is not None:
            for i in range(len(self.controlsptial)):
                c, c_skip = auto_grad_checkpoint(self.controlsptial[i], x, y, t_mlp, c, y_lens, x_mask, t0_mlp, T, S)
                x = auto_grad_checkpoint(self.spatial_blocks[i], x + c_skip, y, t_mlp, y_lens, x_mask, t0_mlp, T, S)
                c, c_skip = auto_grad_checkpoint(self.controltemporal[i], x, y, t_mlp, c, y_lens, x_mask, t0_mlp, T, S)
                x = auto_grad_checkpoint(self.temporal_blocks[i], x + c_skip, y, t_mlp, y_lens, x_mask, t0_mlp, T, S)
            for i in range(len(self.controlsptial),len(self.spatial_blocks)):
                x = auto_grad_checkpoint(self.spatial_blocks[i], x, y, t_mlp, y_lens, x_mask, t0_mlp, T, S)
                x = auto_grad_checkpoint(self.temporal_blocks[i], x, y, t_mlp, y_lens, x_mask, t0_mlp, T, S)
        else:
            for spatial_block, temporal_block in \
                zip(self.spatial_blocks, self.temporal_blocks):
...
@MODELS.register_module("STDiT3-SR")
def STDiT3_XL_SR(from_pretrained=None, **kwargs):
    from opensora.models.controlnet import ControlSTDitBlock
    ...
    if force_huggingface or from_pretrained is not None and not os.path.exists(from_pretrained):
        model = ControlSTDiT3SR.from_pretrained(from_pretrained, **kwargs)
    else:
        config = STDiT3Config(depth=28, hidden_size=1152, patch_size=(1, 2, 2), num_heads=16, **kwargs)
        model = ControlSTDiT3SR(config)
