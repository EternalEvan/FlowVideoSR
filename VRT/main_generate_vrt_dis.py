...
def main():
    ...
    sampler = torch.utils.data.DistributedSampler(test_set)

    test_loader = DataLoader(dataset=test_set, num_workers=args.num_workers, batch_size=1, shuffle=False, sampler=sampler)
    ...
    for idx, batch in enumerate(test_loader):
        ...
        print('Generating {:20s}  ({:2d}/{})'.format(folder[0], idx, len(test_loader)))

    # dist.detroy_process_group()
...
def prepare_model_dataset(args):
    ...
    elif args.task == '001_SimpleVRT_videosr_bi_REDS_6frames':
        model = simplenet(upscale=4, img_size=[6,64,64], window_size=[6,8,8], depths=[8, 4,4,4,4, 4,4],
            indep_reconsts=[11,12], embed_dims=[120, 180,180,180,180, 180,180],
            num_heads=[6, 6,6,6,6, 6,6], pa_frames=2, deformable_groups=12)
        datasets = ['REDS4']
        args.scale = 4
        args.window_size = [6,8,8]
        args.nonblind_denoising = False
